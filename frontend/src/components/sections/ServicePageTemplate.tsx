...
// Code-split heavy interactive animations per route on demand
const DigitalLaunchFlowAnimation = dynamic(
  () => import('@/components/animations/DigitalLaunchFlowAnimation').then((m) => m.DigitalLaunchFlowAnimation),
  { ssr: false }
);

const AutomationIntegrationAnimation = dynamic(
  () => import('@/components/animations/AutomationIntegrationAnimation').then((m) => m.AutomationIntegrationAnimation),
  { ssr: false }
);

const CustomSoftwareAnimation = dynamic(
  () => import('@/components/animations/CustomSoftwareAnimation').then((m) => m.CustomSoftwareAnimation),
  { ssr: false }
);
...
const OperationalProblemSolutionAnimation = dynamic(
  () => import('@/components/animations/OperationalProblemSolutionAnimation').then((m) => m.OperationalProblemSolutionAnimation),
  { ssr: false }
);
...
export function ServicePageTemplate({ service }: ServicePageTemplateProps) {
  ...
  return (
    <div className="py-12 lg:py-20">
      ...
      {/* 2. Service Packages (Basic & Premium) */}
      <ServicePackagesSection service={service} />

      {/* 2. Business Problem & 3. Who it is for (Interactive Pipeline Transformation) */}
      <section className="py-20 bg-surface-card border-y border-border relative overflow-hidden">
        ...
        <Container className="relative z-10">
          <Reveal delay={150}>
            <OperationalProblemSolutionAnimation
              problems={service.problems}
              buyerFit={service.buyerFit}
